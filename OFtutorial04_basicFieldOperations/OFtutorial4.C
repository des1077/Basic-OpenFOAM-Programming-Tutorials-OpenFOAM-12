...
int main(int argc, char *argv[])
{
	...
	(
		...
		// grep -r dimKinematicViscosity $FOAM_SRC/OpenFOAM/
		// This returns:
		...
		/opt/openfoam30/src/OpenFOAM/dimensionSet/dimensionSets.H:extern const dimensionSet dimKinematicViscosity;*/
		// So, it becomes apparent we should check dimensionSets.C, which contain:
		/*const dimensionSet dimLength(0, 1, 0, 0, 0, 0, 0);
		const dimensionSet dimTime(0, 0, 1, 0, 0, 0, 0);
		const dimensionSet dimArea(sqr(dimLength));
		const dimensionSet dimKinematicViscosity(dimArea/dimTime);*/
	...
	(
		...
		(
		    "p", // name of the field
		    runTime.time().name(), // name of the current time, i.e. the time folder to read from; для точного вывода текущего времени используй runTime.time().name(), для округленного значения при выводе в Info используй runTime.userTimeName() )
		    mesh,
